(() => {
  ...
  function isQuestActive(q) {
    return Boolean(q && !q.archived && !ancestorArchived(q));
  }
  ...
  function appendWheelBlocks(svg, blocks, cx, cy, ro, minInner, strokeWidth = 1.5) {
  ...
  function renderWheel() {
    ...
    appendWheelBlocks(svg, blocks, cx, cy, ro, 58, 1.4);
  ...
  function toggleBlockMode({ resetDuration = true } = {}) {
    ...
    if (q && resetDuration) el.blockMinutes.value = String(q.defaultMinutes || 30);
  }
  ...
  function openBlockDialog(questId = null) {
    ...
    el.blockMinutes.value = questId && questById(questId) ? String(questById(questId).defaultMinutes || 30) : "30";
    el.blockName.value = "";
  ...
  function openFinishDialog(blockId) {
    ...
    el.actualMinutes.value = String(b.minutes);
  ...
  el.wheelWrap.addEventListener("pointermove", event => {
    ...
    if (!swipeStart.horizontal && Math.abs(dx) > 8 && Math.abs(dx) > Math.abs(dy) * 1.15) swipeStart.horizontal = true;
  ...
  if ("serviceWorker" in navigator) {
    window.addEventListener("load", async () => {
      try {
        const reg = await navigator.serviceWorker.register("./sw-v4.js", { updateViaCache: "none" });
